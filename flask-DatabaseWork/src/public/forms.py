...
from wtforms.fields import BooleanField, TextField, PasswordField, DateTimeField, IntegerField,SelectField, FloatField, DateField
...
class VstupForm(Form):

    vyrobek = TextField('vyrobek', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=1, max=30, message="Please use between 1 and 30 characters"),
        InputRequired(message="You can't leave this empty")
    ])
    ks = TextField('ks', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=1, max=3, message="Please use between 1 and 3 characters"),
        InputRequired(message="You can't leave this empty")
    ])
    cena = FloatField('cena')
    DPH = IntegerField('DPH')

class gameuserForm(Form):
        username = TextField('Choose your username', validators=[
            Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
            Length(min=6, max=30, message="Please use between 6 and 30 characters"),
            InputRequired(message="You can't leave this empty")
        ])
        email = TextField('Your email', validators=[
            Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
            Length(min=6, max=30, message="Please use between 6 and 30 characters"),
            InputRequired(message="You can't leave this empty")
        ])
        datumprvnihopristupu = DateField("Date first access")
